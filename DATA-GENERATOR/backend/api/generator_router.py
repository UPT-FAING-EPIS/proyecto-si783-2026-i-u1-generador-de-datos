...
def download_file(filename: str, current_user: UsuarioResponse = Depends(get_current_user)):
    """
    Sirve el archivo generado. Requiere autenticación.
    Valida que el archivo esté dentro del directorio temporal permitido.
    """
    # Whitelist de extensiones permitidas
    ALLOWED_EXTENSIONS = {".sql", ".csv", ".json", ".zip"}
    ...
    # Protección contra path traversal: verificar que el path resuelto esté dentro de TEMP_DIR
    resolved_filepath = os.path.realpath(filepath)
    resolved_temp_dir = os.path.realpath(TEMP_DIR)
    if not resolved_filepath.startswith(resolved_temp_dir + os.sep) and resolved_filepath != resolved_temp_dir:
        raise HTTPException(status_code=403, detail="Acceso denegado.")

    if not os.path.exists(resolved_filepath):
        raise HTTPException(status_code=404, detail="Archivo no encontrado.")

    return FileResponse(resolved_filepath, filename=filename)
